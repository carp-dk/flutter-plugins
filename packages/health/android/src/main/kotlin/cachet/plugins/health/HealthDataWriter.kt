...
class HealthDataWriter(
        ...
        /**
         * Writes a single health data record to Health Connect. Supports most basic health metrics
        ...
        /**
         * Writes a comprehensive workout session with optional distance and calorie data. Creates
         * an ExerciseSessionRecord with associated DistanceRecord and TotalCaloriesBurnedRecord if
         * supplementary data is provided.
         *
         * @param call Method call containing workout details: 'activityType', 'startTime',
         * 'endTime',
         * ```
        ...
        fun writeWorkoutData(call: MethodCall, result: Result) {
                ...
                val deviceType: Int? = call.argument<Int>("deviceType")
                val workoutMetadata =
                        buildMetadata(recordingMethod = recordingMethod, deviceType = deviceType)
        ...
        /**
         * Writes blood pressure measurement with both systolic and diastolic values. Creates a
         * single BloodPressureRecord containing both pressure readings taken at the same time
         * point.
         *
         * @param call Method call containing 'systolic', 'diastolic', 'startTime',
         * 'recordingMethod'
         * @param result Flutter result callback returning boolean success status
         */
                        ...
                        try {
                                ...
                                list.add(
                                        NutritionRecord(
                                                ...
                                                endZoneOffset = null,
                                                mealType =
                                                        HealthConstants.mapMealTypeToType[mealType]
         ...
         * containing time-series speed measurements captured during activities like running,
         * cycling, or walking. Each sample represents the user's instantaneous speed at a specific
         * moment within the recording period.
         *
        ...
        fun writeMultipleSpeedData(call: MethodCall, result: Result) {
                ...
                scope.launch {
                        try {
                                                ...
                                                SpeedRecord.Sample(
                                                        ...
                                                        speed =
                                                                Velocity.metersPerSecond(
                                                                        sample["speed"] as Double
                                                                )
                                                )
                                        }

                                val metadata =
                                        buildMetadata(recordingMethod, deviceType = deviceType)
                                ...
                                Log.e(
                                        "FLUTTER_HEALTH::ERROR",
                                        "Error writing speed data: ${e.message}"
                                )
                                result.success(false)
                        }
                }
