...
/**
 * Converts Health Connect records to Flutter-compatible data structures. Handles the transformation
 * of strongly-typed Health Connect data into generic maps that can be serialized and passed to the
 * Flutter layer.
 */
class HealthDataConverter {

    /**
     * Converts a Health Connect record to a list of Flutter-compatible maps. Handles various record
     * types including instant records, interval records, and complex types.
     *
     * @param record The Health Connect record to convert
     * @param dataType The string identifier for the data type being converted
     * @return List<Map<String, Any?>> List of converted records (some records may split into
     * multiple entries)
    ...
    fun convertRecord(
        ...
        return when (record) {
            ...
            is BodyFatRecord ->
                    listOf(createInstantRecord(metadata, record.time, record.percentage.value))
            is LeanBodyMassRecord ->
                    listOf(createInstantRecord(metadata, record.time, record.mass.inKilograms))
            ...
            is RestingHeartRateRecord ->
                    listOf(createInstantRecord(metadata, record.time, record.beatsPerMinute))
            is RespiratoryRateRecord ->
                    listOf(createInstantRecord(metadata, record.time, record.rate))
    ...
    /**
     * Creates a standardized instant record map for point-in-time health measurements. Used for
     * data that represents a single moment measurement (weight, height, etc.).
     *
    ...
    private fun createInstantRecord(
            ...
            createBaseRecord(metadata).apply {
                put("value", value)
    ...
    /**
     * Creates a standardized interval record map for time-range health measurements. Used for data
     * that spans a time period (steps, distance, calories burned, etc.).
     *
     * @param metadata Record metadata containing source and recording information
    ...
    /**
     * Creates the base record structure with common fields shared by all health records. Includes
     * metadata like UUID, source information, and recording method.
     *
    ...
    /**
     * Creates a specialized nutrition record with comprehensive nutrient information. Handles the
     * complex nutrition data structure with multiple nutrient fields, meal type classification, and
     * optional food name.
     *
                    ...
                    .apply {
                        ...
                        putAll(
                                mapOf(
                                        ...
                                        "name" to (record.name ?: ""),
                                        "meal_type" to
                                                (HealthConstants.mapTypeToMealType[record.mealType]
                                                        ?: MEAL_UNKNOWN)
                                )
                        )
                    }
    ...
    /**
     * Converts a sleep stage to a Flutter-compatible map structure. Transforms individual sleep
     * stage data including duration and stage type into a standardized format for Flutter
     * consumption.
     *
